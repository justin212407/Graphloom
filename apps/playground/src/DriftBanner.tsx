...
export default function DriftBanner({ drift, onResolve, onSimulateConflict }: DriftBannerProps) {
  // Extract conflicting node IDs (nodes in BOTH changed sets)
  const conflictingNodeIds: string[] = [];
  if (drift.status === 'conflict') {
    const graphIds = new Set(drift.graphChangedNodeIds);
    // codeChangedSymbols are symbols, not node IDs — but for display we show graphChangedNodeIds
    // The true conflicts are the graphChangedNodeIds since they overlap
        ...
        <div className="drift-conflict-panel">
          ...
          <div className="drift-conflict-nodes">
            {conflictingNodeIds.map(nodeId => (
              <div key={nodeId} className="drift-conflict-node">
